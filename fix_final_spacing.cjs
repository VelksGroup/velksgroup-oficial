...
const replacementCTA = `<div className="flex items-center whitespace-nowrap overflow-hidden">
              {['IA', 'SAAS', 'WEB APPS'].map((tag, idx, arr) => (
                <React.Fragment key={idx}>
                  <span className="text-[10px] font-mono uppercase tracking-tight md:tracking-[0.25em] font-semibold text-[#00F0FF] [text-shadow:0_0_8px_rgba(0,240,255,0.6)] whitespace-nowrap block">
                    {tag}
                  </span>
                  {idx < arr.length - 1 && (
                    <span className="text-[10px] font-mono font-semibold text-[#00F0FF]/50 mx-0 md:mx-2 whitespace-nowrap block">
                      •
                    </span>
                  )}
                </React.Fragment>
  ...
  return (
    ...
    >
      ...
      <div className="pt-5 border-t border-[#D4AF37]/10 mt-auto relative z-10 group-hover:border-[#00F0FF]/30 transition-colors flex items-center whitespace-nowrap overflow-hidden">
        {tags.split(' • ').map((tag, idx, arr) => (
          <React.Fragment key={idx}>
            <span className={\`text-[10px] font-mono uppercase \${isLong ? 'tracking-tighter' : 'tracking-tight'} md:tracking-[0.25em] font-semibold text-[#00F0FF] [text-shadow:0_0_8px_rgba(0,240,255,0.6)] transition-all duration-300 whitespace-nowrap block\`}>{tag}</span>
            {idx < arr.length - 1 && (
              <span className="text-[10px] font-mono font-semibold text-[#00F0FF]/50 mx-0 md:mx-2 whitespace-nowrap block">•</span>
            )}
          </React.Fragment>
        ))}
      </div>
