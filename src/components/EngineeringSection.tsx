...
export const EngineeringSection: React.FC<EngineeringSectionProps> = ({ handleWhatsAppClick, currentLang, t }) => {
  ...
  return (
    <section ref={containerRef} className="relative bg-[#050505] text-white pt-8 pb-20 overflow-hidden">
      <style>
        {`
          @keyframes marquee {
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6">
        <div className="mb-14 flex flex-col md:w-3/4">
          ...
          >
            {t.engineering.title1}<br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#D4AF37] to-[#F1E5AC] font-bold block mt-1">{t.engineering.title2}</span>
          </motion.h2>
        ...
        <div className="mb-16 md:mb-20 max-w-4xl">
          ...
          <div className="grid grid-cols-3 gap-2 md:gap-8">
            <AnimatedMetric
              ...
              duration={1800}
              delay={0}
              label={t.engineering.metrics.automations}
            ...
            <AnimatedMetric
              ...
              duration={1600}
              delay={100}
              label={t.engineering.metrics.aiProjects}
              borderLeft
              formatValue={(val, completed) => <>{val}<span>+</span></>}
            />
            <AnimatedMetric
              ...
              duration={1600}
              delay={200}
              label={t.engineering.metrics.activeInfra}
              borderLeft
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-16 relative z-20">
          <EngineeringCard 
             id="SYS_01"
             title={t.engineering.cards.sys01.title} 
             desc={t.engineering.cards.sys01.desc}
             tags={t.engineering.cards.sys01.tags}
             delay={0.1}
          />
          <EngineeringCard 
             id="SYS_02"
             title={t.engineering.cards.sys02.title} 
             desc={t.engineering.cards.sys02.desc}
             tags={t.engineering.cards.sys02.tags}
             delay={0.2}
          />
          <EngineeringCard 
             id="SYS_03"
             title={t.engineering.cards.sys03.title} 
             desc={t.engineering.cards.sys03.desc}
             tags={t.engineering.cards.sys03.tags}
             delay={0.3}
          />
          <EngineeringCard 
             id="SYS_04"
             title={t.engineering.cards.sys04.title} 
             desc={t.engineering.cards.sys04.desc}
             tags={t.engineering.cards.sys04.tags}
             delay={0.4}
          />
        </div>
      </div>
      ...
      <div className="relative z-10 w-full overflow-hidden border-y border-[#D4AF37]/20 bg-[#050505]/60 py-3 mb-16 flex items-center shadow-[inset_0_0_30px_rgba(212,175,55,0.05)] backdrop-blur-xl">
        ...
        <div className="flex w-max animate-marquee-slow items-center opacity-80 hover:opacity-100 transition-opacity">
          {[...Array(4)].map((_, i) => (
             <span key={i} className="text-[10px] md:text-[11px] font-mono tracking-[0.2em] text-[#D4AF37] font-bold mx-6 uppercase whitespace-nowrap flex items-center [text-shadow:0_0_12px_rgba(212,175,55,0.7)]">
               ...
               SCROLLYTELLING <span className="mx-3 text-[#4FD1FF]/50">•</span> 
               EVENT STREAMING
             </span>
          ))}
        </div>
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 text-center flex flex-col items-center">
        ...
        >
          ...
          <h3 className="text-xl md:text-3xl font-display font-light leading-snug text-white/80 mb-10 text-balance max-w-3xl mx-auto">
            {t.engineering.bottomHero.title1}
            <br className="hidden md:block" />
            <span className="text-white font-medium mt-3 block">{t.engineering.bottomHero.title2}</span>
          </h3>
          
          <div className="flex flex-col items-center gap-4 w-full max-w-[340px] mx-auto md:max-w-none md:w-auto">
            <button 
               onClick={() => handleWhatsAppClick(t.engineering.bottomHero.whatsappMsg)}
            ...
            >
              ...
              <span className="relative z-10 flex items-center gap-3">
                {t.engineering.bottomHero.cta}
                <ChevronRight size={16} className="group-hover:translate-x-1 transition-transform" />
              </span>
            </button>
            <div className="inline-flex items-center justify-center gap-2 md:gap-3 whitespace-nowrap">
              {t.engineering.bottomHero.ctaTags.map((tag, idx, arr) => (
                <React.Fragment key={idx}>
                  <span className="text-[11px] md:text-xs font-mono uppercase tracking-widest font-semibold text-[#00F0FF] [text-shadow:0_0_8px_rgba(0,240,255,0.6)] whitespace-nowrap block">
                    {tag}
                  </span>
                  {idx < arr.length - 1 && (
                    <span className="text-[11px] md:text-xs font-mono font-semibold text-[#00F0FF]/50 whitespace-nowrap block">
                      •
                    </span>
                  )}
                </React.Fragment>
...
const EngineeringCard = ({ id, title, desc, tags, delay }: { id: string, title: string, desc: string, tags: string, delay: number }) => {
  ...
  return (
    <motion.div 
      ...
      className="tech-card flex flex-col p-4 md:p-8 rounded-xl group"
    >
      {/* Technical Corners */}
      ...
      <div className="pt-5 border-t border-[#D4AF37]/10 mt-auto relative z-10 group-hover:border-[#00F0FF]/30 transition-colors flex items-center justify-between w-full px-1 md:px-2 whitespace-nowrap overflow-hidden">
        {tags.split(' • ').map((tag, idx, arr) => (
          <React.Fragment key={idx}>
            <span className={`text-[10px] md:text-xs font-mono uppercase ${isLong ? 'tracking-tighter md:tracking-[0.25em]' : 'tracking-normal md:tracking-[0.25em]'} font-semibold text-[#00F0FF] [text-shadow:0_0_8px_rgba(0,240,255,0.6)] transition-all duration-300 whitespace-nowrap block`}>{tag}</span>
            {idx < arr.length - 1 && (
              <span className="text-[10px] md:text-xs font-mono font-semibold text-[#00F0FF]/50 whitespace-nowrap block">•</span>
            )}
          </React.Fragment>
        ))}
      </div>
